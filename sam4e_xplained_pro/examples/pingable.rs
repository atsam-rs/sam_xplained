...
use cortex_m_systick_countdown::{CountsMillis, PollingSysTick, SysTickCalibration};
...
use smoltcp::socket::{RawPacketMetadata, RawSocketBuffer, SocketSet, SocketSetItem};
...
static mut RXDESCRIPTORTABLE: Aligned<A4, ethernet::RxDescriptorTable<RXDESCRIPTOR_COUNT>> =
    Aligned(ethernet::RxDescriptorTable::new());
static mut TXDESCRIPTORTABLE: Aligned<A4, ethernet::TxDescriptorTable<TXDESCRIPTOR_COUNT>> =
    Aligned(ethernet::TxDescriptorTable::new());
...
fn main() -> ! {
    ...
    let mut interface = EthernetInterfaceBuilder::new(eth)
        ...
        .ethernet_addr(smoltcp::wire::EthernetAddress::from_bytes(
            &ethernet_address.as_bytes(),
        ))
    ...
    let mut dhcp = Dhcpv4Client::new(
        ...
        dhcp_rx_buffer,
        dhcp_tx_buffer,
        Instant::from_millis(ticker.count().0 as i64),
    );
    ...
    loop {
        ...
        if previous_link_state.is_none() || link_state != previous_link_state {
            if link_state.is_some() {
                hprintln!(
                    "Ethernet link is now UP with {} Mbps.",
                    link_state.unwrap().0
                )
                .unwrap();
            } else {
        ...
        if link_state.is_some() {
            let timestamp = Instant::from_millis(ticker.count().0 as i64);
            interface
            ...
            let _config = dhcp
                .poll(&mut interface, &mut sockets, timestamp)
                .unwrap_or_else(|e| {
